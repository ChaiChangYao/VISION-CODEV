...
export async function seedDemo(connectionString?: string): Promise<void> {
  ...
  try {
    ...
    await withTenantContext(pool, { companyId: DEMO_IDS.company, memberId: DEMO_IDS.member }, async (db) => {
      const content = { states: [{ id: 'ready', label: 'Ready' }], steps: [{ id: 'fold-1', title: 'Make the first fold', instruction: 'Fold the paper corner to the center crease.' }] };
      await db.query(
      ...
      await db.query(
        ...
        [DEMO_IDS.capture, DEMO_IDS.company, DEMO_IDS.workflow, `company-${DEMO_IDS.company}-golden-run`],
      );
      await db.query(
        ...
        [DEMO_IDS.graph, DEMO_IDS.company, DEMO_IDS.workflow, content, createHash('sha256').update(JSON.stringify(content)).digest('hex')],
      );
    });
  } finally {
    await pool.end();
...
if (process.argv[1]?.endsWith('seed.ts')) seedDemo().catch((error: unknown) => { console.error(error); process.exitCode = 1; });
