...
export class WorkflowRepository {
  ...
  public async create(input: {
    ...
    const result = await this.db.query<WorkflowRecord>(
      ...
      [
        ...
        this.db.tenant.companyId,
        this.db.tenant.memberId,
        input.name,
        input.family,
      ],
    );
  ...
  public async list(): Promise<WorkflowRecord[]> {
    const result = await this.db.query<WorkflowRecord>(
      'SELECT * FROM workflows WHERE company_id = $1 ORDER BY updated_at DESC, id DESC',
      [this.db.tenant.companyId],
    );
    return result.rows;
  }
  ...
  public async get(id: string): Promise<WorkflowRecord | undefined> {
    const result = await this.db.query<WorkflowRecord>(
      'SELECT * FROM workflows WHERE company_id = $1 AND id = $2',
      [this.db.tenant.companyId, id],
    );
    return result.rows[0];
  }
}
...
export class CaptureRepository {
  ...
  public async create(
    workflowId: string,
    input: { livekitRoom?: string; id?: string } = {},
  ...
  public async get(id: string): Promise<CaptureSessionRecord | undefined> {
    const result = await this.db.query<CaptureSessionRecord>(
      'SELECT * FROM capture_sessions WHERE company_id = $1 AND id = $2',
      [this.db.tenant.companyId, id],
    );
    return result.rows[0];
  }
  ...
  public async addMediaAsset(
    sessionId: string,
    input: { objectKey: string; state?: MediaAssetRecord['state']; id?: string },
