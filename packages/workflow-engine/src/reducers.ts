...
const captureTransitions: Record<CaptureState, Partial<Record<CaptureEvent['type'], CaptureState>>> = {
...
export function reduceCaptureState(state: CaptureState, event: CaptureEvent): ReducerResult<CaptureState> {
...
const deploymentTransitions: Record<DeploymentState, Partial<Record<DeploymentEvent['type'], DeploymentState>>> = {
  draft: { READY: 'ready', FAIL: 'failed' },
  ready: { START: 'starting', FAIL: 'failed' },
  starting: { GUIDE: 'guiding', FAIL: 'failed' },
  guiding: { PAUSE: 'paused', INTERRUPT: 'interrupted', DEVIATION_DETECTED: 'recovery_required', COMPLETE: 'completed', FAIL: 'failed' },
...
export function reduceDeploymentState(state: DeploymentState, event: DeploymentEvent): ReducerResult<DeploymentState> {
