...
const approvedProvenance = new Set([
  'EXPERT_ASSERTION',
  'REVIEWER_CORRECTION',
...
export function normalizeProcedureGraph(graph: ProcedureGraph): ProcedureGraph {
  ...
  if (!parsed.success) {
    throw new GraphValidationError([issue('SCHEMA_INVALID', 'graph', parsed.error.message)]);
  }
...
export function validateProcedureGraph(graph: ProcedureGraph): GraphValidationResult {
  ...
  try {
    ...
    if (error instanceof GraphValidationError)
      return { valid: false, publishable: false, issues: error.issues };
    throw error;
  }
  ...
  normalized.steps.forEach((step, index) => {
    ...
    if (step.ordinalHint < previousOrdinal) {
      issues.push(
        issue(
          ...
          `steps[${index}].ordinalHint`,
          'Steps must have non-decreasing ordinal hints.',
        ),
      );
    }
    ...
    for (const stateId of [...step.startState, ...step.endState]) {
      if (!stateIds.has(stateId)) {
        issues.push(
          issue('UNKNOWN_STATE', `steps[${index}]`, `Unknown procedure state: ${stateId}.`),
        );
      }
    }
    for (const recoveryId of step.recoveryTransitions) {
      if (
        !stepIds.has(recoveryId) &&
        !normalized.steps.some((candidate) => candidate.id === recoveryId)
      ) {
        issues.push(
          issue(
            'UNKNOWN_RECOVERY',
            `steps[${index}].recoveryTransitions`,
            `Unknown recovery transition: ${recoveryId}.`,
          ),
        );
      }
    }
    if (
      step.provenance.includes('MODEL_INFERENCE') &&
      !step.provenance.some((value) => approvedProvenance.has(value))
    ) {
      issues.push(
        issue(
          'UNAPPROVED_INFERENCE',
          `steps[${index}].provenance`,
          'Model inference requires expert assertion, reviewer correction, or published requirement before it can guide execution.',
          normalized.published ? 'error' : 'warning',
        ),
      );
    }
    if (normalized.published && !step.provenance.includes('PUBLISHED_REQUIREMENT')) {
      issues.push(
        issue(
          ...
          `steps[${index}].provenance`,
          'Published steps require PUBLISHED_REQUIREMENT provenance.',
        ),
      );
    }
  });
