...
describe('deterministic procedure engine', () => {
  ...
  it('does not publish model inference without publication provenance', () => {
    const result = validateProcedureGraph({ ...graph, published: true, steps: [{ ...graph.steps[0]!, provenance: ['MODEL_INFERENCE'] }] });
  ...
  it('enforces capture and deployment state transitions', () => {
    ...
    expect(reduceDeploymentState('guiding', { type: 'DEVIATION_DETECTED', recoveryId: 'r', reason: 'wrong' }).state).toBe('recovery_required');
    expect(reduceDeploymentState('guiding', { type: 'RECOVERY_STARTED', recoveryId: 'r' }).accepted).toBe(false);
  });

  it('publishes only with explicit publication provenance', () => {
    const published = publishProcedureGraph({ ...graph, steps: [{ ...graph.steps[0]!, provenance: ['EXPERT_ASSERTION', 'PUBLISHED_REQUIREMENT'] }] });
