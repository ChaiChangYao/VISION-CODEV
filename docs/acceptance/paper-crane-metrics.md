...
The workflow engine exposes `evaluatePaperCraneAcceptance(report)`. It returns `accepted: true` only when the dataset gate is satisfied, every threshold in this document passes, p95 intervention latency is measured and at most 2 seconds, at least 10 approved recovery fixtures all pass, replay determinism is 100%, and every recorded `Why?` response has approved provenance. Missing evidence or zero-denominator metrics remain unaccepted.
