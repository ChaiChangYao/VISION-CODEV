...
1. Configure `EXPO_PUBLIC_LIVEKIT_URL`, `EXPO_PUBLIC_CAPTURE_TOKEN_ENDPOINT` (for local API: `/v1/capture-token`), `EXPO_PUBLIC_CAPTURE_PAIRING_ENDPOINT` (for local API: `/v1/capture-pairings/claim`), `EXPO_PUBLIC_COMPANY_ID`, `EXPO_PUBLIC_MEMBER_ID`, `EXPO_PUBLIC_PAIRING_CODE`, and a stable `EXPO_PUBLIC_DEVICE_ID` in a local, uncommitted environment. The desktop Train surface displays the six-digit code after preparing a capture; the native client claims it before requesting a publisher token. Legacy explicit `EXPO_PUBLIC_WORKFLOW_ID`/`EXPO_PUBLIC_SESSION_ID` configuration remains available only for controlled fixture/debug use.
