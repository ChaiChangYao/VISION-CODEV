...
export function PhoneCaptureScreen() {
  ...
  const session = useMemo(() => {
    ...
    const tokenEndpoint = stringValue(extra?.captureTokenEndpoint) ?? process.env.EXPO_PUBLIC_CAPTURE_TOKEN_ENDPOINT;
  ...
  useEffect(() => {
    if (!session) {
      setMessage('Configure EXPO_PUBLIC_LIVEKIT_URL and EXPO_PUBLIC_CAPTURE_TOKEN_ENDPOINT for a physical-device build.');
      return;
    }
    ...
    <OrientationSafeCaptureView>
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <View style={[styles.statusDot, snapshot.connection === 'connected' && styles.statusDotLive]} />
        </View>

        <View style={styles.preview}>
          <Text style={styles.previewLabel}>{snapshot.state === 'active' ? 'CAPTURING' : 'READY'}</Text>
        ...
        {message || snapshot.error ? <Text style={styles.message}>{message ?? snapshot.error}</Text> : null}

        <View style={styles.controls}>
          <Pressable style={styles.secondaryButton} onPress={() => void session?.switchCamera()} disabled={!session || snapshot.state !== 'active'}>
          ...
          {snapshot.state === 'active' || snapshot.state === 'paused' ? (
            ...
            <Pressable style={styles.startButton} onPress={() => void start()} disabled={!session || snapshot.state === 'preparing'}>
              <Text style={styles.startButtonText}>{snapshot.state === 'failed' ? 'Retry' : 'Start capture'}</Text>
            </Pressable>
          )}
        </View>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  preview: { flex: 1, minHeight: 260, marginVertical: 20, borderRadius: 24, padding: 24, justifyContent: 'flex-end', backgroundColor: '#102238', borderWidth: 1, borderColor: '#213b56' },
  ...
  secondaryButton: { flex: 1, minHeight: 54, borderRadius: 14, borderWidth: 1, borderColor: '#3d526d', justifyContent: 'center', alignItems: 'center' },
  secondaryButtonText: { color: '#d5e0ee', fontSize: 15, fontWeight: '600' },
  startButton: { flex: 1, minHeight: 54, borderRadius: 14, backgroundColor: '#73e6c3', justifyContent: 'center', alignItems: 'center' },
  startButtonText: { color: '#07151b', fontSize: 15, fontWeight: '800' },
  stopButton: { flex: 1, minHeight: 54, borderRadius: 14, backgroundColor: '#ff8d83', justifyContent: 'center', alignItems: 'center' },
