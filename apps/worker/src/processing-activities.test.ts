...
const media = { companyId: '00000000-0000-7000-8000-000000000001', objectKey: 'companies/captures/source.mp4', kind: 'media' as const };
...
describe('Temporal processing activity boundary', () => {
  it('validates handler artifacts and carries object references only', async () => {
    const handlers: ProcessingActivityHandlers = {
      ...
      async transcribe(input) { return { companyId: input.companyId, objectKey: 'companies/transcript.json', kind: 'transcript' }; },
      async extractObservations(input) { return { companyId: input.companyId, objectKey: 'companies/observations.json', kind: 'observations' }; },
      async induceProcedure(input) { return { companyId: input.companyId, objectKey: 'companies/procedure.json', kind: 'procedure-draft' }; },
    };
    ...
    expect(transcript).toEqual({ companyId: context.companyId, objectKey: 'companies/transcript.json', kind: 'transcript' });
  });
});
