...
export function ConnectedGoldenRun({ workflowId, stage }: { workflowId: string; stage: Stage }) {
  ...
  const publish = async () => { try { const value = await api.publishProcedure(workflowId, { graph: { ...graph, steps: graph.steps.map((step, index) => index === selectedStep ? { ...step, instruction } : step) }, reviewerNote: 'Reviewed and approved in the Approve surface.' }); setGraph(value); setMessage('Procedure published as an immutable version.'); } catch (value) { reportError(value); } };
