...
import { issueLiveKitToken, liveKitRoomName } from './livekit-token.js';
...
import { claimDeploymentPairing, createDeploymentPairing, pairingIsActive } from './deployment-pairing.js';
...
import { currentApprovedStep, evaluateTechnicianObservation } from './technician-guidance.js';
import { startLocalGuidanceProcess, stopLocalGuidanceProcesses } from './guidance-process.js';
...
function deploymentView(run: DeploymentRun) { const workflow = getWorkflow(run.workflowId, run.companyId); const graph = workflow.graph; const step = graph?.steps[run.currentStep]; const why = graph && step ? WhyResponseSchema.parse({ text: `This instruction is taken from approved procedure version ${graph.version}.`, evidenceIds: step.evidenceRefs, procedureVersion: graph.version, provenance: step.provenance }) : undefined; return { ...run, status: run.status === 'active' ? 'monitoring' : run.status, totalSteps: graph?.steps.length ?? 0, currentInstruction: step?.instruction, currentStepContext: graph ? currentApprovedStep(run, graph) : undefined, intervention: run.intervention, why }; }
...
function captureView(capture: CaptureSession) {
  return {
    ...
    roomName: liveKitRoomName({ companyId: capture.companyId, workflowId: capture.workflowId, sessionId: capture.id }),
...
async function monitorView(capture: CaptureSession, companyId: string, memberId: string) {
  ...
  return {
    ...
    roomName: liveKitRoomName({ companyId, workflowId: capture.workflowId, sessionId: capture.id }),
...
async function route(request: IncomingMessage, response: ServerResponse) {
  ...
  if (path === '/v1/internal/guidance-token' && method === 'POST') {
    ...
    const view = deploymentView(deployment);
    send(response, 200, { ...issued, deploymentId, observationPath: `/v1/deployments/${deploymentId}/observations`, currentInstruction: view.currentInstruction, currentStep: view.currentStepContext }, traceId);
    return;
  }
  ...
  if (parts[0] === 'deployment-token' && parts.length === 1 && method === 'POST') {
    const deploymentId = String(payload.deploymentId ?? ''); const workflowId = String(payload.workflowId ?? ''); const deviceId = String(payload.deviceId ?? '');
    const deployment = getDeployment(deploymentId, companyId);
    if (workflowId !== deployment.workflowId) throw new HttpError(403, 'FORBIDDEN_TENANT', 'The deployment does not belong to the requested workflow.');
    if (String(payload.memberId ?? '') !== memberId) throw new HttpError(403, 'FORBIDDEN_TENANT', 'The token member must match the authenticated request member.');
    if (!deployment.pairedDeviceId || !deviceId || deviceId !== deployment.pairedDeviceId) throw new HttpError(403, 'FORBIDDEN_TENANT', 'The phone must claim this tutorial before a publisher token is issued.');
    if (!pairingIsActive(deployment)) throw new HttpError(401, 'UNAUTHENTICATED', 'The tutorial pairing has expired. Start a new deployment.');
    const workflow = getWorkflow(deployment.workflowId, companyId);
    if (!workflow.graph?.published || workflow.status !== 'Published') throw new HttpError(412, 'PRECONDITION_FAILED', 'A published procedure is required before the phone can join.');
    if (!liveKitConfigured) throw new HttpError(503, 'EXTERNAL_PROVIDER_UNAVAILABLE', 'LiveKit credentials are not configured.');
    const issued = await issueLiveKitToken({ companyId, memberId, workflowId: deployment.workflowId, sessionId: deployment.id, role: 'publisher', expiresInSeconds: 3600 });
    deployment.status = 'active';
    try {
      const guidance = startLocalGuidanceProcess({ deploymentId: deployment.id, workflowId: deployment.workflowId, companyId, memberId });
      deployment.guidanceStatus = guidance === 'externally_managed' ? 'externally_managed' : 'running';
    } catch (error) {
      deployment.guidanceStatus = 'failed';
      throw new HttpError(503, 'EXTERNAL_PROVIDER_UNAVAILABLE', error instanceof Error ? error.message : 'Realtime guidance could not start.');
    }
    send(response, 200, { ...issued, currentInstruction: deploymentView(deployment).currentInstruction }, traceId); return;
  }
  ...
  if (parts[0] === 'capture-pairings' && parts[1] === 'claim' && parts.length === 2 && method === 'POST') { const pairingCode = String(payload.pairingCode ?? '').trim(); const deviceId = String(payload.deviceId ?? '').trim(); if (!/^\d{6}$/.test(pairingCode) || !deviceId) throw new HttpError(400, 'VALIDATION_FAILED', 'A six-digit pairingCode and deviceId are required.'); const capture = [...store.captures.values()].find((value) => value.companyId === companyId && value.pairingCode === pairingCode); if (!capture?.pairingExpiresAt || Date.parse(capture.pairingExpiresAt) <= Date.now()) throw new HttpError(401, 'UNAUTHENTICATED', 'The pairing code is invalid or expired.'); if (capture.pairedDeviceId && capture.pairedDeviceId !== deviceId) throw new HttpError(409, 'CONFLICT', 'This capture session is already paired to another device.'); capture.pairedDeviceId = deviceId; capture.pairedAt ??= now(); event(companyId, 'capture.paired', { deviceId }, capture.workflowId, capture.id); send(response, 200, { sessionId: capture.id, workflowId: capture.workflowId, deviceId, roomName: liveKitRoomName({ companyId, workflowId: capture.workflowId, sessionId: capture.id }) }, traceId); return; }
  if (parts[0] === 'deployment-pairings' && parts[1] === 'claim' && parts.length === 2 && method === 'POST') {
    const pairingCode = String(payload.pairingCode ?? '').trim(); const deviceId = String(payload.deviceId ?? '').trim();
    if (!/^\d{6}$/.test(pairingCode) || !deviceId) throw new HttpError(400, 'VALIDATION_FAILED', 'A six-digit pairingCode and deviceId are required.');
    const deployment = [...store.deployments.values()].find((value) => value.companyId === companyId && value.pairingCode === pairingCode);
    if (!deployment) throw new HttpError(401, 'UNAUTHENTICATED', 'The tutorial pairing code is invalid or expired.');
    try { Object.assign(deployment, claimDeploymentPairing(deployment, deviceId)); }
    catch (error) { const message = error instanceof Error ? error.message : 'Tutorial pairing failed.'; throw new HttpError(message.includes('another device') ? 409 : 401, message.includes('another device') ? 'CONFLICT' : 'UNAUTHENTICATED', message); }
  ...
  if (parts[0] === 'workflows' && parts[2] === 'deployments' && parts.length === 3 && method === 'POST') { const workflow = getWorkflow(parts[1]!, companyId); if (workflow.status !== 'Published') throw new HttpError(412, 'PRECONDITION_FAILED', 'A published workflow is required before deployment.'); const pairing = createDeploymentPairing(store.deployments.values()); const run: DeploymentRun = { id: id(), workflowId: workflow.id, companyId, status: 'ready', currentStep: 0, deviations: [], paperState: { status: 'clear' }, voiceState: 'closed', ...pairing }; store.deployments.set(run.id, run); event(companyId, 'deployment.created', { status: run.status, pairingExpiresAt: run.pairingExpiresAt! }, workflow.id, undefined, run.id); send(response, 201, deploymentView(run), traceId); return; }
  if (parts[0] === 'workflows' && parts[2] === 'deployments' && parts[3] === 'current' && parts.length === 4 && method === 'GET') {
    getWorkflow(parts[1]!, companyId);
    const deployment = [...store.deployments.values()].filter((value) => value.companyId === companyId && value.workflowId === parts[1]).at(-1);
    if (!deployment) throw new HttpError(404, 'NOT_FOUND', 'No deployment has been started for this workflow.');
    send(response, 200, deploymentView(deployment), traceId); return;
  }
  ...
  if (parts[0] === 'deployments' && parts[2] === 'monitor' && parts.length === 3 && method === 'GET') {
    const run = getDeployment(parts[1]!, companyId);
    const token = liveKitConfigured ? await issueLiveKitToken({ companyId, memberId, workflowId: run.workflowId, sessionId: run.id, role: 'viewer', expiresInSeconds: 3600 }) : undefined;
    send(response, 200, { sessionId: run.id, roomName: liveKitRoomName({ companyId, workflowId: run.workflowId, sessionId: run.id }), serverUrl: process.env.LIVEKIT_URL ?? '', viewerToken: token?.token ?? '', status: liveKitConfigured ? 'waiting' : 'disconnected', audioConnected: false, videoConnected: false, reason: liveKitConfigured ? 'Waiting for tutorial phone.' : 'LiveKit credentials are not configured.' }, traceId); return;
  }
  ...
  if (parts[0] === 'deployments' && parts[2] === 'observations' && parts.length === 3 && method === 'POST') {
    const run = getDeployment(parts[1]!, companyId); const workflow = getWorkflow(run.workflowId, companyId);
    if ('assessment' in payload) {
      if (!workflow.graph?.published) throw new HttpError(412, 'PRECONDITION_FAILED', 'A published procedure is required for task evaluation.');
      let decision; try { decision = evaluateTechnicianObservation(run, workflow.graph, payload); }
      catch (error) { throw new HttpError(400, 'VALIDATION_FAILED', error instanceof Error ? error.message : 'Invalid technician observation.'); }
      event(companyId, `guidance.${decision.decision.toLowerCase()}`, { reason: decision.decisionReason, step: run.currentStep }, run.workflowId, undefined, run.id);
      send(response, 200, { ...deploymentView(run), ...decision }, traceId); return;
    }
...
// Large recordings may take longer than Node's five-minute upload deadline.
// Keep a finite deadline; saveImportedMedia still enforces the byte ceiling.
server.requestTimeout = 30 * 60 * 1000;
server.listen(port, () => console.log(`Vision Codef API listening on http://localhost:${port}`));
process.on('SIGTERM', () => { stopLocalGuidanceProcesses(); void store.close(); server.close(); });
