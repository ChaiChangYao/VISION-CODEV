...
export type DeploymentRun = { id: string; workflowId: string; companyId: string; status: 'ready' | 'active' | 'paused' | 'completed' | 'failed'; currentStep: number; deviations: Array<{ id: string; state: 'candidate' | 'confirmed' | 'recovering' | 'resolved' | 'overridden' | 'unresolved'; severity: string }>; paperState?: DeviationState; intervention?: Record<string, unknown>; voiceState?: VoiceState; pairingCode?: string; pairingExpiresAt?: string; pairedDeviceId?: string; pairedAt?: string; guidanceStatus?: 'starting' | 'running' | 'externally_managed' | 'failed'; guidanceCandidate?: { stepId: string; kind: 'completed' | 'deviation'; count: number; sinceTimestampMs: number; lastTimestampMs: number }; lastObservation?: Record<string, unknown> };
