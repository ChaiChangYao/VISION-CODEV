import type { CaptureState, EventEnvelope, ProcedureGraph, WorkflowFamily } from '@vision-codef/contracts';
...
export type CaptureSession = { id: string; workflowId: string; companyId: string; state: CaptureState; startedAt?: string; endedAt?: string; mediaAssetId?: string };
...
export class DevelopmentStore { readonly workflows = new Map<string, Workflow>(); readonly captures = new Map<string, CaptureSession>(); readonly deployments = new Map<string, DeploymentRun>(); readonly events: EventEnvelope[] = []; }
